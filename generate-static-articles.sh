...
do
    ...
    do
        ...
        authors=$(jq -c -r '.authors[]|{givenName: .givenNames[], familyName:.familyNames[], email:.emails[]}' $articleFolder/article.json || jq -c -r '.authors[]|{givenName: .givenNames[], familyName:.familyNames[]}' $articleFolder/article.json)
        authorsHtml=""
        oldIFS="$IFS"
        IFS=$'\n'
